...
- S Timekeeping Accuracy ±5ppm (±0.432 Second/ Day) from -45°C to +85°C
- S Battery Backup for Continuous Timekeeping
- S Low Power Consumption
- S Footprint and Functionally Compatible to DS3231
...
- S Fast (400kHz) I^2 C-Compatible Serial Bus
- S +2.3V to +5.5V Supply Voltage
- S Digital Temp Sensor with ±3°C Accuracy
- S -45°C to +85°C Temperature Range
- S 8-Pin or 16-Pin SO Packages
...
| Supply Voltage | V_BAT    | 2.3       | 3.0   | 5.5       | V       |
...
|-------------------------------------------------------|----------|--------------|-------|-------|-------|---------|
| Active Supply Current 2                               | ICCA     | VCC = +3.63V |       |       |   200 | µA      |
| (I C Active: Includes Temperature Conversion Current) | ICCA     | VCC = VCCMAX |       |       |   300 | µA      |
...
| PARAMETER                             | SYMBOL   | CONDITIONS           | MIN   | TYP   | MAX   | UNITS   |
|---------------------------------------|----------|----------------------|-------|-------|-------|---------|
| Power-Fail Voltage                    | V_PF     |                      | 2.45  | 2.575 | 2.70  | V       |
...
| V_BAT Leakage                         | I BATLKG |                      |       | 25    | 100   | nA      |
| Temperature Accuracy                  | TEMPACC  | VCC or V_BAT = +3.3V |       | Q 3   |       | N C     |
...
| PARAMETER                                                      | SYMBOL   | CONDITIONS                     | MIN   | TYP   |   MAX | UNITS   |
|----------------------------------------------------------------|----------|--------------------------------|-------|-------|-------|---------|
| Active Battery Current (I^2 C Active) (Note 4)                 | I BATA   | V_BAT = +3.63V                 |       |       |  70   | µA      |
| Active Battery Current (I^2 C Active) (Note 4)                 | I BATA   | V_BAT = V BATMAX               |       |       | 150   | µA      |

=== PDF PAGE 3 ===
| Timekeeping Battery Current (I^2 C Inactive) (Note 4)          | I BATT   | V_BAT = +3.63V, EN32KHZ = 0    |       | 2     |   3   | µA      |
| Timekeeping Battery Current (I^2 C Inactive) (Note 4)          | I BATT   | V_BAT = V BATMAX , EN32KHZ = 0 |       | 2     |   3.5 | µA      |
| Temperature Conversion Current (I^2 C Inactive)                | I BATTC  | V_BAT = +3.63V                 |       |       | 575   | µA      |
| Temperature Conversion Current (I^2 C Inactive)                | I BATTC  | V_BAT = V BATMAX               |       |       | 650   | µA      |
| Data Retention Current (Oscillator Stopped and I^2 C Inactive) | I BATDR  | T_A = +25 N C                  |       |       | 100   | nA      |
...
| Bus Free Time Between STOP and START Conditions | t_BUF    |              | 1.3         |       |       | F s     |
| Hold Time (Repeated) START Condition            | t_HD:STA |              | 0.6         |       |       | F s     |

=== PDF PAGE 4 ===
| Low Period of SCL                               | tLOW     |              | 1.3         |       |       | F s     |
| High Period of SCL                              | t_HIGH   |              | 0.6         |       |       | F s     |
| Data Hold Time                                  | t_HD:DAT |              | 0           |       | 0.9   | F s     |
| Data Set-Up Time                                | t_SU:DAT |              | 100         |       |       | ns      |
| START Set-Up Time                               | t_SU:STA |              | 0.6         |       |       | F s     |
| SDA and SCL Rise Time                           | t_R      | (Note 7)     | 20 + 0.1C B |       | 300   | ns      |
| SDA and SCL Fall Time                           | t_F      | (Note 7)     | 20 + 0.1C B |       | 300   | ns      |
| STOP Set-Up Time                                | t_SU:STO |              | 0.6         |       |       | F s     |
...
| 4     | 4     | RST       | Active-Low Reset. This pin is an open-drain input/output. It indicates the status of VCC relative to the V_PF specification. As VCC falls below V_PF , the RST pin is driven low. When VCC exceeds V_PF , for t RST , the RST pin is pulled high by the internal pullup resistor. The active-low, open-drain output is combined with a debounced pushbutton input function. This pin can be activated by a pushbutton reset request. It has an internal 50k I (R PU ) nominal value pullup resistor to V_CC. No external pullup resistors should be connected. If the oscillator is disabled, t REC is bypassed and RST immediately goes high.          |
| -     | 5-12  | N.C.      | No Connection. These pins must be connected to ground.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

=== PDF PAGE 7 ===
| 5     | 13    | GND       | Ground                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 6     | 14    | V_BAT     | Backup Power-Supply Input. When using the device with the V_BAT input as the primary power source, this pin should be decoupled using a 0.1 F F to 1.0 F F low-leakage capacitor. When using the device with the V_BAT input as the backup power source, the capacitor is not required. If V_BAT is not used, connect to ground. The device is UL recognized to ensure against reverse charging when used with a primary lithium battery. Go to www.maximintegrated.com/qa/info/ul for more information.                                                                                                                                                |
...
|-------|-------|--------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 8 SO  | 16 SO | NAME   | FUNCTION                                                                                                                                                                                                                       |
| 7     | 15    | SDA    | Serial-Data Input/Output. This pin is the data input/output for the I^2 C serial interface. This open-drain pin requires an external pullup resistor. The pullup voltage can be up to 5.5V, regardless of the voltage on V_CC. |
| 8     | 16    | SCL    | Serial-Clock Input. This pin is the clock input for the I^2 C serial interface and is used to synchronize data movement on the serial interface. The pullup voltage can be up to 5.5V, regardless of the voltage on V_CC.      |
...
| CONFIGURATION          | CONDITION   | I/O ACTIVE   | I/O ACTIVE   | I/O INACTIVE   | I/O INACTIVE   | RST             |
|------------------------|-------------|--------------|--------------|----------------|----------------|-----------------|
| VCC Only (Figure 4)    | VCC > V_PF  |              |              |                |                | Inactive (High) |
| VCC Only (Figure 4)    | VCC < V_PF  | ICCA         | ICCA         | I CCS          | I CCS          | Active (Low)    |
| V_BAT Only (Figure 5)  | EOSC = 0    |              |              | I BATT         | I BATT         | Disabled (Low)  |
| V_BAT Only (Figure 5)  | EOSC = 1    | I BATA       | I BATA       | I BATDR        | I BATDR        | Disabled (Low)  |
| Dual Supply (Figure 6) | VCC > V_PF  | ICCA         | ICCA         | I CCS          | I CCS          | Inactive (High) |
| Dual Supply (Figure 6) | VCC < V_PF  | VCC > V_BAT  | ICCA         | VCC > V_BAT    | I CCS          | Active (Low)    |
| Dual Supply (Figure 6) | VCC < V_PF  | VCC < V_BAT  | I BATA       | VCC < V_BAT    | I BATT         | Active (Low)    |
...
## ±5ppm, I^2 C Real-Time Clock DS3231M
...
| 0Dh       | A2M4        | DY/ DT     | Date            | Date       | Day     | Day     | Day     | Day         | Alarm 2 Day     | 1-7                 |
| 0Dh       | A2M4        | DY/ DT     | 10              | 10         | Date    | Date    | Date    | Date        | Alarm 2 Date    | 1-31                |
...
| BIT 7    | EOSC : Enable oscillator. When set to logic 0, the oscillator is started. When set to logic 1, the oscillator is stopped when the device switches to V_BAT . This bit is clear (logic 0) when power is first applied. When the device is powered by V_CC, the oscillator is always on regardless of the status of the EOSC bit. When the oscil- lator is disabled, all register data is static.                                                                                                                                                                                                                                                                                     |
|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| BIT 6    | BBSQW: Battery-backed square-wave enable. When set to logic 1 with INTCN = 0 and VCC < V_PF , this bit enables the 1Hz square wave. When BBSQW is logic 0, INT /SQW goes high impedance when VCC falls below V_PF . This bit is disabled (logic 0) when power is first applied.                                                                                                                                                                                                                                                                                                                                                                                                     |
...
Slave  Address  Byte: Each  slave  on  the  I^2 C  bus responds to a slave address byte sent immediately following a START condition. The slave address byte contains the slave address in the most significant 7
